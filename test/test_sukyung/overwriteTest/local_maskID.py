...
with open(fname, 'r', encoding='UTF8') as f:
    ...
    for Word in f.readlines():
        ...
        #주민번호 패턴 탐지
        patternID = re.compile("(?P<ID>\d{6})[-](?P<maskID>\d{7})")
        ...
        '''
        #핸드폰번호 패턴 탐지
        patternP = re.compile("(\d{2})[-]\d{2}[-]\d{6}[-]\d{2}")
        msgP = patternP.search(Word)

        #외국인등록번호 패턴 탐지
        patternF = re.compile("(\d{2})[-]\d{2}[-]\d{6}[-]\d{2}")
        msgF = patternF.search(Word)

        #신용카드번호 패턴 탐지
        patternC = re.compile("(\d{2})[-]\d{2}[-]\d{6}[-]\d{2}")
        msgC = patternC.search(Word)

        #건강보험번호 패턴 탐지
        patternHI = re.compile("(\d{2})[-]\d{2}[-]\d{6}[-]\d{2}")
        msgHI = patternHI.search(Word)

        #계좌번호 패턴 탐지
        patternA = re.compile("(\d{2})[-]\d{2}[-]\d{6}[-]\d{2}")
